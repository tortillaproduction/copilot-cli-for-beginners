...
> **もしAIがコードベース全体を一度に見ることができたら？**

このチャプターでは、GitHub Copilot CLIの本当の力を引き出します：コンテキストです。`@`シンタックスを使ってファイルとディレクトリを参照する方法を学び、Copilot CLIにコードベースの深い理解を与えます。セッションをまたいで会話を維持し、数日後に中断したところから正確に再開する方法、そして単一ファイルレビューでは見落とされるバグを複数ファイル解析で発見する方法を学びます。

## 🎯 学習目標

このチャプターの終了までに、以下ができるようになります：

- `@`シンタックスを使ってファイル、ディレクトリ、画像を参照する
- `--resume`と`--continue`で前のセッションを再開する
- [context windows](../GLOSSARY.md#context-window)の仕組みを理解する
- 複数ターンの効果的な会話を書く
- 複数プロジェクトのワークフローでディレクトリ権限を管理する

> ⏱️ **所要時間**: 約50分（読書20分 + ハンズオン30分）
...
*あなたの同僚と同じく、Copilot CLIは読心術者ではありません。より多くの情報を提供することで、人間とCopilot CLIの両方がより的確なサポートを提供できるようになります！*

バグを同僚に説明するシーンを想像してください：

> **コンテキストなし**: 「書籍アプリが動きません」

> **コンテキストあり**: 「`books.py`の`find_book_by_title`関数を見てください。大文字小文字を区別しないマッチングをしていません」

Copilot CLIにコンテキストを提供するには、`@`シンタックスを使ってCopilot CLIを特定のファイルに指し示します。
...
このセクションではコンテキストを効果的に使うために必要なすべてをカバーしています。まずこれらの基本をマスターしましょう。

---

## @シンタックス

`@`シンボルはプロンプト内のファイルとディレクトリを参照します。これはCopilot CLIに「このファイルを見てください」と伝える方法です。

> 💡 **注**: このコースのすべての例は、このリポジトリに含まれている`samples/`フォルダを使っているので、すべてのコマンドを直接試すことができます。

### 今すぐ試してみる（セットアップ不要）

コンピュータ上の任意のファイルで試すことができます：
...
> 💡 **プロジェクトを用意していませんか？** クイックテストファイルを作成してみてください：
> ```bash
...
<details>
<summary>🎬 実際に動作する様子を見る！</summary>

![File Context Demo](images/file-context-demo.gif)

*デモ出力は異なります。あなたのモデル、ツール、レスポンスは表示されているものと異なる場合があります。*
...
ここでコンテキストが真の力を発揮します。単一ファイル分析は有用です。複数ファイル分析は変革的です。
...
> 💡 **高度なオプション**: セキュリティに焦点を当てた複数ファイル分析については、Pythonセキュリティ例を試してみてください：
> ```bash
...
1. データフローパターン
   book_app.pyはBookCollectionインスタンスを作成しメソッドを呼び出す
   books.pyはBookCollectionクラスを定義しデータの永続性を管理する
...
2. 重複する表示関数
   ...
   影響: ほぼ同じ処理をしている2つの関数が存在。
   1つを更新する（形式を変更するなど）場合、
   もう1つも更新することを忘れてはいけません。

3. 矛盾したエラーハンドリング
   book_app.pyは年の変換からValueErrorを処理する
   books.pyはエラー時にNone/Falseを黙って返す

   パターン: モジュール全体で統一されたエラーハンドリング手法がない
```

**これが重要な理由**: 単一ファイルレビューではより大きな全体像を見落とします。複数ファイル分析だけが以下を明かします：
- 統合すべき**重複コード**
- コンポーネントがどう相互作用するかを示す**データフローパターン**
...
プロジェクトは初めてですか？Copilot CLIを使ってすばやく学びましょう。
...
**得られるもの**:
```
これはJSON ファイルに保存された本を追加、リスト表示、削除、検索できるCLI書籍コレクションマネージャーです。最大の品質上の問題は以下の通りです：

1. 重複する表示ロジック - show_books()とprint_books()が同じことをしている
2. 矛盾したエラーハンドリング - いくつかのエラーは例外を発生させ、他はFalseを返す
3. 入力検証がない - yearが0でも、title/authorが空の文字列でも受け入れられる
4. テストがない - find_book_by_titleなどのクリティカルな関数にテストカバレッジがない

優先度の高い修正: 重複する表示関数を統合し、入力検証を追加する。
```

**結果**: コード読書に1時間かかるところが10秒に圧縮されます。どこに焦点を当てるべき正確に分かります。
